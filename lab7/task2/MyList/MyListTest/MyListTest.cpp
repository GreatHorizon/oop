...
BOOST_FIXTURE_TEST_SUITE(My_list, EmptyIntList)
	...
	BOOST_AUTO_TEST_CASE(can_be_incremented_when_it_is_last_element)
	{
		auto it = list.end();
		--it;
	...
	BOOST_AUTO_TEST_CASE(cant_be_decremented_when_it_is_iterator_with_first_element)
	{
		auto it = list.begin();
		BOOST_CHECK_THROW(--it, std::runtime_error);
	}
	...
	BOOST_AUTO_TEST_SUITE(reversed_iterator)
		...
		BOOST_AUTO_TEST_CASE(can_be_used_with_range_based_for)
		{
			...
			BOOST_CHECK_EQUAL_COLLECTIONS(receivedList.rbegin(), receivedList.rend(),
				expectedList.rbegin(), expectedList.rend());
		}
...
BOOST_FIXTURE_TEST_SUITE(My_string_list, StringList)
	BOOST_AUTO_TEST_SUITE(string_list_test)
		BOOST_AUTO_TEST_CASE(element_can_be_added_to_the_list)
		{
			...
			list.Append("7");
			BOOST_CHECK_EQUAL(*list.rbegin(), "7");
